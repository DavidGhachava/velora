...
export function HomePage() {
  ...
  const { properties, roomTypes } = catalog.data
  const { t, formatNumber } = useLocale()
  const areaCount = new Set(properties.map((hotel) => hotel.area)).size
  const roomCount = properties.reduce((total, hotel) => total + hotel.roomCount, 0)
  const apartmentStartingRate = Math.min(...properties.filter((hotel) => hotel.propertyType === 'Apartment').map((hotel) => hotel.startingRateGel ?? Number.POSITIVE_INFINITY))
  ...
  return <>
    ...
    <section className="section section--hotel-discovery"><div className="content-container">
      <div className="discovery-heading"><div><p className="eyebrow">{t('Batumi accommodation')}</p><h2>{t('Hotels and apartments')}</h2></div><div className="discovery-stats"><div><strong>{properties.length}</strong><span>{t('properties')}</span></div><div><strong>{areaCount}</strong><span>{t('areas')}</span></div><div><strong>{formatNumber(roomCount)}</strong><span>{t('rooms')}</span></div></div></div>
      ...
      <div className="hotel-grid hotel-grid--featured">{properties.slice(0, 3).map((hotel) => <HotelCard key={hotel.slug} hotel={hotel} />)}</div>
      <div className="section-action"><Link className="button button--secondary button--lg" to="/hotels">{t('View all properties')}</Link></div>
    </div></section>
    <section id="featured-rooms" className="section section--sand"><div className="content-container"><div className="compact-section-heading"><div><p className="eyebrow">Velora Batumi</p><h2>{t('Available room types')}</h2></div><Link className="inline-action" to="/rooms">{t('View all rooms')} <ArrowRight size={16} /></Link></div><div className="room-grid">{roomTypes.map((room) => <RoomCard key={room.id} room={room} />)}</div></div></section>
  </>
}
