...
export function StaySearch({ className = '', defaults, propertySlug }: { className?: string; defaults?: Partial<SearchValues>; propertySlug?: string }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<SearchValues>({
    ...
    defaultValues: { checkIn: defaults?.checkIn ?? '2026-08-14', checkOut: defaults?.checkOut ?? '2026-08-17', guests: defaults?.guests ?? 2 },
  })
  ...
  return <form className={`search-panel ${className}`} onSubmit={handleSubmit(onSubmit)} aria-label={t('Search room availability')}>
    <div className="field"><label htmlFor="search-checkin">{t('Arrival')}</label><input id="search-checkin" className="date-picker-input" type="date" min="2026-08-11" onClick={showDatePicker} {...register('checkIn')} />{errors.checkIn && <p className="field-error">{t(errors.checkIn.message ?? '')}</p>}</div>
    <div className="field"><label htmlFor="search-checkout">{t('Departure')}</label><input id="search-checkout" className="date-picker-input" type="date" min="2026-08-12" onClick={showDatePicker} {...register('checkOut')} />{errors.checkOut && <p className="field-error">{t(errors.checkOut.message ?? '')}</p>}</div>
